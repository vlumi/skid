...
public enum PieceCompiler {
    ...
    public enum Grid {
        ...
        ///
        /// Within a row cars are spread evenly about the centerline, so a 3-row
        /// is left/centre/right and a 2-row is the staggered pair. Pole is the
        /// outermost car of the front row, as in a real grid.
        public static func positions(line: Vec2, dir: Vec2, count: Int = slots) -> [Vec2] {
            ...
            for (row, width) in rows(for: count).enumerated() {
                ...
                for seat in 0..<width {
                    ...
                    out.append(line - dir * back + inward * offset)
                }
            }
            return out
        }
