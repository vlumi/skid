...
- [ ] **The track browser — the editor can only open ONE track.** The library already stores many (`TrackLibraryBook` keeps names, dates, signatures, `isRaceable`), but `editorLayout` is a single buffer restored from a single slot; edits flow *to* the library and nothing flows back. **The hazard is in-place mutation**: `syncEditedTrackToLibrary` rewrites the edited entry on every change, so "open track X" would silently overwrite X — which makes two distinct acts, both wanted: **edit this track** and **start from this track** (a copy, which is what opening a built-in must do). *Scratchpads need no new storage*: an unfinished track is already an entry with `isRaceable == false`; what is missing is seeing and opening them — a list plus new / duplicate / rename / delete, and where thumbnail previews earn their place. Also still open from the editor overhaul: a **long-press variant picker**, for deleting and reassigning the start line.
