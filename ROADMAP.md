...
- [ ] **Verify sharing on real devices.** Links, QR and scanning all shipped, but three parts can only be checked on hardware: a universal link resolving through the deployed AASA (the simulator exercises the handler, not the lookup), the camera scanner itself, and whether scanning somebody's screen actually works at arm's length in normal light.
