...
case "$platform" in
    ios)   scheme="Skid-iOS";   destination="generic/platform=iOS";   ext="ipa" ;;
    macos) scheme="Skid-macOS"; destination="generic/platform=macOS"; ext="pkg" ;;
    *) echo "usage: distribute.sh <ios|macos> [--no-upload|--upload-only]" >&2; exit 2 ;;
esac

out="dist/${platform}"
archive="${out}/Skid-${platform}.xcarchive"
